/* ============================================================
   SALSA SIKIRI — JS principal
   ============================================================ */

// --- Datos de contacto centralizados ---
const SIKIRI = {
  whatsapp: "5215510112861",
  email: "salsa.sikiri@gmail.com",
  instagram: "https://www.instagram.com/salsa_sikiri/",
};

// Abre WhatsApp con un mensaje. `msg` opcional.
function abrirWhatsApp(msg) {
  const texto = msg || "¡Hola Salsa Sikiri! Quiero más información sobre sus productos.";
  const url = `https://wa.me/${SIKIRI.whatsapp}?text=${encodeURIComponent(texto)}`;
  window.open(url, "_blank");
}

// Handler del formulario "Enviar por WhatsApp"
function enviarPorWhatsApp(e) {
  e.preventDefault();
  abrirWhatsApp();
}

// Menú hamburguesa (se reasigna tras inyectar el header)
function toggleMenu() {
  const navLinks = document.querySelector(".nav-links");
  if (navLinks) navLinks.classList.toggle("active");
}

// Marca el link activo según la página actual
function setActiveNav() {
  const path = location.pathname.split("/").pop() || "index.html";
  document.querySelectorAll(".nav-links a").forEach((a) => {
    const href = a.getAttribute("href");
    if (href === path || (path === "" && href === "index.html")) a.classList.add("active");
  });
}

// Inyecta header y footer compartidos + botón flotante de WhatsApp
async function injectPartials() {
  const tryFetch = async (file, elId) => {
    const el = document.getElementById(elId);
    if (!el) return;
    try {
      const res = await fetch(file);
      el.innerHTML = await res.text();
    } catch (err) {
      console.warn("No se pudo cargar", file, err);
    }
  };

  await tryFetch("header.html", "site-header");
  await tryFetch("footer.html", "site-footer");
  setActiveNav();

  // Botón flotante de WhatsApp (en todas las páginas)
  if (!document.querySelector(".wa-float")) {
    const wa = document.createElement("a");
    wa.className = "wa-float";
    wa.href = `https://wa.me/${SIKIRI.whatsapp}`;
    wa.target = "_blank";
    wa.rel = "noopener";
    wa.setAttribute("aria-label", "Escríbenos por WhatsApp");
    wa.innerHTML =
      '<svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path d="M.057 24l1.687-6.163a11.867 11.867 0 01-1.587-5.945C.16 5.335 5.495 0 12.05 0a11.817 11.817 0 018.413 3.488 11.824 11.824 0 013.48 8.414c-.003 6.557-5.338 11.892-11.893 11.892a11.9 11.9 0 01-5.688-1.448L.057 24zm6.597-3.807c1.676.995 3.276 1.591 5.392 1.592 5.448 0 9.886-4.434 9.889-9.885.002-5.462-4.415-9.89-9.881-9.892-5.452 0-9.887 4.434-9.889 9.884a9.86 9.86 0 001.51 5.26l-.999 3.648 3.978-1.044zm11.387-5.464c-.074-.124-.272-.198-.57-.347-.297-.149-1.758-.868-2.031-.967-.272-.099-.47-.149-.669.149-.198.297-.768.967-.941 1.165-.173.198-.347.223-.644.074-.297-.149-1.255-.462-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.521.151-.172.2-.296.3-.495.099-.198.05-.372-.025-.521-.075-.148-.669-1.611-.916-2.206-.242-.579-.487-.501-.669-.51l-.57-.01c-.198 0-.52.074-.792.372s-1.04 1.016-1.04 2.479 1.065 2.876 1.213 3.074c.149.198 2.096 3.2 5.077 4.487.709.306 1.263.489 1.694.626.712.226 1.36.194 1.872.118.571-.085 1.758-.719 2.006-1.413.247-.694.247-1.289.173-1.413z"/></svg>';
    document.body.appendChild(wa);
  }
}

document.addEventListener("DOMContentLoaded", () => {
  injectPartials();

  // Animaciones
  if (window.AOS) AOS.init({ duration: 900, once: true, offset: 60 });

  // Carrusel de reseñas (solo si existe en la página)
  if (window.Swiper && document.querySelector(".swiper")) {
    new Swiper(".swiper", {
      speed: 700,
      loop: true,
      autoplay: { delay: 5000, disableOnInteraction: false },
      slidesPerView: 1,
      spaceBetween: 24,
      centeredSlides: true,
      pagination: { el: ".swiper-pagination", clickable: true },
      breakpoints: {
        768: { slidesPerView: 2 },
        1024: { slidesPerView: 3 },
      },
    });
  }
});
